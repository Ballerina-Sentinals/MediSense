...
service /reminders on reminderListener {
    resource function post createReminder(Reminder newReminder) returns ReminderCreated |error? {
        ...
        var recordContent = newReminder.recordInfo; // Assuming record_id contains the record content

        // Insert into Records table
        sql:ParameterizedQuery insertRecordQuery = `INSERT INTO Records (record) VALUES (${recordContent})`;
        var insertResult = check dbClient->execute(insertRecordQuery);

        //Capture the integer or string generated by the database in response to a query execution.
        string|int? recordId = insertResult.lastInsertId;

         // Prepare the query to insert the reminder
        sql:ParameterizedQuery insertReminderQuery = `INSERT INTO Reminders (user_id, reminder_date, reminder_time, record_id)
                                                      VALUES (${userId}, ${date}, ${time}, ${recordId})`;

        // Execute the query
        var result = check dbClient->execute(insertReminderQuery);
